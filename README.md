...
`Select L` and `Select R` select left-side or right-side curve objects in the current mirror collection scope. If curve objects are selected, the scope is their direct collections and child collections are not included. If no curve is selected, selected collections are used together as recursive roots. If no collection is selected, the active collection is used as the recursive root. Non-curve objects in the scope are ignored.
