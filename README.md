...
`Select L` and `Select R` select left-side or right-side curve objects in the current mirror collection scope. Selected Outliner collections are used together as recursive roots even when the button is pressed from the sidebar. If no collection is selected, selected curve objects limit the scope to their direct collections and child collections are not included. If neither collection nor curve is selected, the active collection is used as the recursive root. Non-curve objects in the scope are ignored.
